...
from lib.test_utils.mockexchange import MockExchange, MockTicker
from lib import json as customjson
from lib.db.dbsync import Base
from lib.exchanges import EXCHANGES, EXCHANGE_TICKERS
...
class Messages:
    ...
    def create(cls, *channels: Channel, messenger: Messenger):
        ...
        return cls(
            ...
            results={str(c.ns) + str(c.topic): loop.create_future() for c in channels},
            messenger=messenger,
        )
    ...
    async def __aenter__(self):
        async def register_channel(channel: Channel):
            ...
            await self.messenger.sub_channel(channel.ns, channel.topic, callback)
