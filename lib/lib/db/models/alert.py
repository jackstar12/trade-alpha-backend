...
class Alert(Base, Serializer, BaseMixin):
    ...
    def get_discord_embed(self):
        embed = (
            discord.Embed(title="Alert")
            .add_field(name="Symbol", value=self.symbol)
            .add_field(name="Price", value=self.price)
            .add_field(name="Exchange", value=self.exchange)
        )
        if self.note:
