...
from lib.db.dbsync import fkey_name

if TYPE_CHECKING:
    from lib.db.models.balance import Balance as BalanceDB
...
class EventScore(Base, Serializer, BaseMixin):
    ...
    entry_id = Column(
        ForeignKey(
            "evententry.id",
            name=fkey_name("eventscore", "entry_id"),
            ondelete="CASCADE",
        ),
        primary_key=True,
    )
    ...
    def gain(self):
        if self.rel_value and self.abs_value:
            return Gain.construct(relative=self.rel_value, absolute=self.abs_value)
...
class EventEntry(Base, Serializer, BaseMixin):
    ...
    user_id = Column(
        ForeignKey(
            "user.id", name=fkey_name("evententry", "user_id"), ondelete="CASCADE"
        ),
        nullable=False,
    )
    client_id = Column(
        ForeignKey(
            "client.id", name=fkey_name("evententry", "client_id"), ondelete="SET NULL"
        ),
        nullable=True,
    )
    event_id = Column(
        ForeignKey(
            "event.id", name=fkey_name("evententry", "event_id"), ondelete="CASCADE"
        ),
        nullable=False,
    )
    init_balance_id = Column(
        ForeignKey(
            "balance.id",
            name=fkey_name("evententry", "init_balance_id"),
            ondelete="CASCADE",
        ),
        nullable=True,
    )
...
EventEntry.rank_history = relationship(
    EventScore, lazy="noload", order_by=asc(EventScore.time)
)
