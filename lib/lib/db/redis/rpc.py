...
class Server:
    ...
    async def parse_request(self, req_data):
        ...
        try:
            ...
            await self.send_response(
                req_id, None, BadRequest("missing request key", key)
            )
            return
        if method not in self.methods:
            logger.error("BadRequest: method not found: %s", method)
            await self.send_response(
                req_id, None, BadRequest("method not found", method)
            )
            return
        if type(params) != dict:
    ...
    async def call_method(self, req_id, method_name: str, params: dict):
        ...
        try:
            val = await utils.return_unknown_function(method.fn, method.model(**params))
        except Exception as e:
            logger.exception("CallError: %s", e)
            await self.send_response(req_id, None, CallError(repr(e)))
            return
        logger.info(
            "Success: method=%s, params=%s, result=%s", method_name, params, val
        )
