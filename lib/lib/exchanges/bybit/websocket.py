...
class BybitWebsocketClient(WebsocketManager):
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            session=session, get_url=get_url, ping_forever_seconds=30, **kwargs
        )
    ...
    async def authenticate(self, api_key: str, api_secret: str):
        ...
        sign = str(
            hmac.new(
                api_secret.encode("utf-8"), _val.encode("utf-8"), digestmod="sha256"
            ).hexdigest()
        )
    ...
    async def _on_message(self, ws: WebsocketManager, message: dict):
        ...
        if op:
            ...
            await utils.return_unknown_function(self._on_data_message, self, message)
