...
import lib.db.models.balance as db_balance
...
class BitmexWorker(Exchange):
    ...
    async def _get_balance(self, time: datetime, upnl=True):
        ...
        response = await self.get("/api/v1/user/margin", params={"currency": "all"})
        ...
        for currency in response:
            ...
            if self.usd_like(symbol):
                ...
                response_price = await self.get(
                    "/api/v1/trade",
                    params={"symbol": symbol.upper(), "count": 1, "reverse": "True"},
                )
            ...
            extra_currencies.append(
                db_balance.Amount(
                    currency=symbol, realized=realized_amount, unrealized=unrealized
                )
            )
    ...
    def _sign_request(
        ...
        request_signature = (
            f'{method}{path}{f"?{query_string}" if query_string else ""}{ts}'
        )
        if data is not None:
            request_signature += data
        signature = hmac.new(
            self.client.api_secret.encode(), request_signature.encode(), "sha256"
        ).hexdigest()
