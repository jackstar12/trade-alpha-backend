...
def event_dep(*eager, require_user=False):
    ...
    async def dependency(
        ...
        assoc: EventGrant = Depends(
            get_table_auth_dependency(
                (EventGrant.event, event_loads), association_table=EventGrant
            )
        ),
        ...
        if not assoc.event:
            if assoc.grant.is_root_for(AssociationType.EVENT):
                stmt = select(EventDB).filter(
                    EventDB.id == event_id, EventDB.owner_id == assoc.grant.author_id
                )
...
async def create_event(
    ...
    if event.location.name != "web":
        active_event = await dbutils.get_event(
            location=body.location, throw_exceptions=False, db=db
        )

        if active_event:
            if body.start < active_event.end:
                raise BadRequest(
                    f"Event can't start while other event ({active_event.name}) is still active"
                )
        ...
        active_registration = await dbutils.get_event(
    ...
    return OK(result=EventInfo.from_orm_with(event, extra={"grants": []}))


EventUserDep = get_current_user((User.events, EventDB.actions))
    ...
    return OK(
        result=[
            EventInfo.from_orm_with(
                ...
                extra={
                    "grants": event.grants
                    if grant.is_root_for(AssociationType.EVENT)
                    else [grant]
                },
            )
...
@wrap_greenlet
def get_event(
    grant: EventGrant = Depends(
        event_dep(
            EventDB.owner,
            (
                EventDB.entries,
                [EventEntryDB.client, EventEntryDB.user, EventEntryDB.init_balance],
            ),
        )
    )
):
    return OK(result=EventDetailed.from_orm_with(grant.event, extra={"grant": grant}))


SummaryEvent = event_dep(
    (EventDB.entries, [EventEntryDB.client, EventEntryDB.init_balance])
)


@router.get("/{event_id}/leaderboard", response_model=ResponseModel[Leaderboard])
async def get_event_leaderboard(
    date: Optional[datetime] = None, grant: EventGrant = Depends(SummaryEvent)
):
...
async def get_summary(
    grant: EventGrant = Depends(SummaryEvent), date: Optional[datetime] = None
):
...
async def get_event_entry_history(
    ...
    scores = await db_all(
        select(EventScoreDB)
        .where(EventScoreDB.entry_id.in_(entry_ids), EventEntryDB.event_id == event_id)
        .join(EventScoreDB.entry)
        .order_by(asc(EventScoreDB.time)),
        session=db,
    )
...
@router.get(
    "/{event_id}/registrations/{entry_id}",
    response_model=ResponseModel[EventEntryDetailed],
    dependencies=[Depends(EventAuth)],
)
async def get_event_entry(
    event_id: int, entry_id: int, db: AsyncSession = Depends(get_db)
):
    score = await db_unique(
        select(EventEntryDB).filter_by(id=entry_id, event_id=event_id),
...
class EventUpdate(BaseModel):
    ...
    def cant_update_past(cls, v):
        ...
        if v and v < now:
            raise ValueError("Can not udpate date from the past")
        return v
...
async def delete_event(
    grant: EventGrant = Depends(event_dep(EventDB.actions, require_user=True)),
    db: AsyncSession = Depends(get_db),
):
    if grant.event:
...
async def join_event(
    ...
    if client_id:
        try:
            entry = EventEntryDB(
                event_id=grant.event.id, client_id=client_id, user=user
            )
            db.add(entry)
            await db.commit()
...
async def unregister_event(
    ...
    result = await db.execute(stmt.filter_by(id=entry_id) if entry_id else stmt)
    await db.commit()
