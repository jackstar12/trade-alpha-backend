...
def admin_only(coro, cog=True):
    ...
    async def wrapper(*args, **kwargs):
        ...
        if ctx.author.guild_permissions.administrator:
            ...
            await ctx.send(
                "This command can only be used by administrators", hidden=True
            )
    ...
    def decorator(coro):
        ...
        async def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if e.user_id:
                    if ctx.guild:
                        e.reason = e.reason.replace(
                            "{name}", ctx.guild.get_member(e.user_id).display_name
                        )
                    else:
                ...
                await ctx.send(
                    f"This is a bug in the bot. Please contact jacksn#9149. ({e.reason})",
                    hidden=True,
                )
                logging.error(
                ...
                if ctx.deferred:
                    await ctx.send(
                        "This is a bug in the bot. Please contact jacksn#9149.",
                        hidden=True,
                    )
                logging.critical(
...
def gradient_fill(x, y, fill_color=None, ax=None, **kwargs):
    ...
    lines = []
    colors = []
    for index, ((prev_x, prev_y), (now_x, now_y)) in enumerate(
        itertools.pairwise(zip(x, y))
    ):
        if math.copysign(1, prev_y) != math.copysign(1, now_y):
            new_line = np.column_stack(
                (x[current_cut : index + 1], y[current_cut : index + 1])
            )
    ...
    def add_gradient(xmin, xmax, ymin, ymax, color, inverse):
        ...
        z[:, :, :3] = rgb
        z[:, :, -1] = np.linspace(
            alpha if inverse else 0, 0 if inverse else alpha, 100
        )[:, None]
        ...
        z[:, :, -1] = np.array(
            [
                np.power(x_val, 1.2)
                for x_val in (reversed(alpha_values) if inverse else alpha_values)
            ]
        )[:, None]
        im = ax.imshow(
            z,
...
async def get_summary_embed(event: dbmodels.Event, dc_client: discord.Client):
    ...
    if summary:
        for name, user_id in [
            ...
            embed.add_field(
                name=name,
                value=user.discord.get_display_name(dc_client, event.guild_id),
                inline=False,
            )
    ...
    *,
    ...
    if True:
        for registered_client, name in to_graph:
            ...
            history = await dbutils.get_client_history(
                registered_client,
            ...
            if history.data:
                ...
                for prev_item, item, next_item in core.prev_now_next(
                    ...
                    if isinstance(item, Balance):
                        ...
                        if percentage:
                            if relative_to_amount:
                                amount = calc_percentage(
                                    relative_to_amount, current, string=False
                                )
                            else:
                                amount = 0
                        else:
                    ...
                    if amount is not None and (
                        not next_item or next_item.time != item.time
                    ):
                        xs.append(item.time)
                        if include_upnl:
            ...
            if mode == "balance" or len(to_graph) > 1:
                ...
                gradient_fill(
                    xs, np.array([float(y) for y in ys]), fill_color="green", alpha=0.55
                )
...
async def get_leaderboard_embed(
    event: dbmodels.Event, leaderboard: Leaderboard, dc_client: discord.Client
):
    footer = ""
    description = ""
    ...
    logging.info(
        f"Done creating leaderboard. Description:\n" f"{de_emojify(description)}"
    )

    return discord.Embed(title="Leaderboard :medal:", description=description)
        ...
        if len(args) > 0:
            for arg in args:
                try:
                    ...
                    raise UserInputError(f"Invalid time argument: {arg}")
        date = now - timedelta(
            hours=hour, minutes=minute, days=day, weeks=week, seconds=second
        )
...
async def ask_for_consent(
    ...
    msg_embeds: Optional[List[discord.Embed]] = None,
    yes_message: Optional[str] = None,
    no_message: Optional[str] = None,
    hidden=False,
    timeout_seconds: float = 60,
) -> Future[Tuple[ComponentContext, bool]]:
    ...
    await ctx.send(content=msg_content, embeds=msg_embeds, components=[component_row])

    return await asyncio.wait_for(future, timeout_seconds)


def create_yes_no_button_row(
    slash: SlashCommand,
    author_id: int,
    yes_callback: Optional[Callable] = None,
    no_callback: Optional[Callable] = None,
    yes_message: Optional[str] = None,
    no_message: Optional[str] = None,
    hidden=False,
) -> Dict:
    ...
    buttons = [
        create_button(style=ButtonStyle.green, label="Yes", custom_id=yes_id),
        create_button(style=ButtonStyle.red, label="No", custom_id=no_id),
    ]
...
async def new_create_selection(
    ctx: SlashContext,
    slash: SlashCommand,
    ...
    **kwargs,
    ...
    component_row = create_selection(
        ...
        ctx.author_id,
        options,
        callback=lambda component_ctx, selections: future.set_result(
            (component_ctx, selections)
        ),
        **kwargs,
    )
...
def select_client(
    ctx, dc: discord.Client, slash: SlashCommand, user: DiscordUser, max_values=None
):
...
def get_embed(fields: Optional[dict] = None, **embed_kwargs):
